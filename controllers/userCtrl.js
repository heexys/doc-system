...
const getAllDoctorsController = async (req, res) => {
    try{
        ...
        res.status(200).send({
            success:true,
            message:'Doctors List Fetched Succesfull',
            data:doctors,
        })
    } catch (error) {
        console.log(error)
        res.status(500).send({
            success:false,
            error,
            message:'Error While Fetched Doctor',
        })
    }
}
...
const bookeAppointmentController = async (req,res) => {
    try {
        ...
        res.status(200).send({
            success:true,
            message:'Appointment Book Succefully'
        })
    } catch (error) {
        console.log(error)
        res.status(500).send({
...
const bookingAvailabilityController = async (req,res) => {
    try{
        ...
        if(appointments.length > 0){
            return res.status(200).send({
                message:'Appointments Available not Availale at this time',
                success:false
            })
        }else {
            return res.status(200).send({
